...
#     # loop.create_task(video(request))
#     # loop.close()
#     # loop.run_forever()
#     # loop.close()
#     # result = await(video(request))
#     # print(result)
#     # result = sync_to_async(video(request), thread_sensitive=True)
#
#     loop = asyncio.get_event_loop()
#     asyncio.create_task(video(request))
#     asyncio.run(video(request))
#     # asyncio.ensure_future(video(request))
#     # loop.close()
...
def post(request):
    ...
    video(userKey, groupCode, qzGroup, fileKey, qzNum, jobCode, a1, fileUrl)
    return Response("Done", status=200)